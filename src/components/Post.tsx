...
export default function Post({ post, onOpenBetModal, onStatusChange, onPostDeleted }: PostProps) {
  ...
  const [editedOdds, setEditedOdds] = useState(post.odds.toString());
  const { user } = useAuth();
  ...
  const getMatchesText = (content: string) => {
    const lines = content.split('\n');
    return lines.filter(line => line.match(/(.+) vs (.+) - (.+): (.+)/)).join('\n');
  };

  const analysisText = getAnalysisText(post.content);
  const matchesText = getMatchesText(post.content);
  ...
  const handleSaveEdit = async () => {
    try {
      const odds = parseFloat(editedOdds);
      if (isNaN(odds)) {
        throw new Error('La cote doit être un nombre valide');
      }
        ...
        {isEditing ? (
          <div className="space-y-4">
            ...
            <div>
              ...
              <textarea
                value={getAnalysisText(editedContent)}
                onChange={(e) => {
                  const newContent = getMatchesText(editedContent) + '\n\n' + e.target.value;
                  setEditedContent(newContent);
                }}
                className="w-full rounded-lg border border-gray-300 p-3 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                rows={4}
